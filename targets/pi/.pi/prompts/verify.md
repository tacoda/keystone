...
Read `harness/process/verification.md`. The IRON LAW applies: **no completion claims without fresh verification evidence**. Run every sensor in this turn. Stale output from a previous turn does not count.

Steps:

1. Read the tool commands from `harness/state/CODEBASE_STATE.md` (lint, type-check, test, build, coverage).
...
5. Read the diff and propose state-layer updates (coverage delta, region recency) as a diff against `harness/state/CODEBASE_STATE.md`. Do not silently overwrite — propose, then confirm.
