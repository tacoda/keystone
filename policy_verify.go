...
// Violations are hard errors (strict cascade broken). Gaps are informational
// — items a policy says should exist somewhere but no tier has provided.
type VerifyResult struct {
	Violations []ShadowViolation
	Gaps       []MissingRequired
}

// HasErrors reports whether any strict rule was violated. Gaps are advisory
// and do not flip this flag — the user can ship without them, but the audit
// surfaces them so they get filled in.
...
// ShadowViolation reports one strict item that is being overridden by a
// lower tier. Paths are recorded relative to the install dir, slash-separated.
...
// String renders the violation for terminal output. Each violation prints
// the declaring policy, the locked item, and every offending path.
...
// verifyPolicies walks the lockfile and reports both strict-item shadowing
// (hard violations) and unmet required-item declarations (advisory gaps).
// The returned VerifyResult is always non-nil. Errors here are I/O failures,
// not policy violations.
func verifyPolicies(installDir string) (*VerifyResult, error) {
	...
	for _, policyName := range sortedPolicyNames(lf.Policies) {
		...
		for _, kind := range []string{"guides", "playbooks", "actions", "sensors"} {
			...
			for _, item := range strictItemsFor(lock.Required, kind) {
				defined, walkErr := isItemDefined(installDir, kind, item, lf.Policies)
				if walkErr != nil {
					return nil, walkErr
				}
				if defined {
...
func isItemDefined(installDir, kind, item string, policies map[string]PolicyLock) (bool, error) {
...
// basename matches `<item>.md` under the kind subtree. The declaring
// policy's own files are excluded (a policy never shadows itself).
//
...
func findShadowing(installDir, kind, item, declaringTier string, policies map[string]PolicyLock, declaringName string) ([]string, error) {
	var hits []string
...
// findItemInTree walks subdir under installDir and returns relative paths
// of every file named `<item>.md`. Returns nil (not an error) when subdir
// is absent.
...
func sortedPolicyNames(m map[string]PolicyLock) []string {
...
func printVerifyReport(installDir string, res *VerifyResult) bool {
