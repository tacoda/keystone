...
`<charter-root>` is the charter folder name configured in `keystone.json`
(default `.charter` at 2.0; was `charter` in 1.x). `<repo-root>`
is the project root — the directory that contains `keystone.json`.

## Directory layout (2.0)

Every charter consumer ends up with the same tree regardless of which
...
- **`context.yaml`** is interface-flavored config (MCP topic bindings,
  optional CLI projection registry), not charter content. Absent
  means defaults.
...
- **Project path:** `<charter-root>/guides/<topic>/<name>.md`
- **Policy path:** `<charter-root>/policies/<policy>/guides/<topic>/<name>.md`
...
- **Project path:** `<charter-root>/corpus/<topic>/<name>.md`
- **Policy path:** `<charter-root>/policies/<policy>/corpus/<topic>/<name>.md`
...
- **Project path:** `<charter-root>/sensors/<name>.md`
- **Policy path:** `<charter-root>/policies/<policy>/sensors/<name>.md`
...
- **Project path:** `<charter-root>/actions/<name>.md`
- **Policy path:** `<charter-root>/policies/<policy>/actions/<name>.md`
...
- **Project path:** `<charter-root>/playbooks/<name>.md`
- **Policy path:** `<charter-root>/policies/<policy>/playbooks/<name>.md`
...
- **Project path:** `<charter-root>/adapters/<agent>/{lifecycle,sensors,activation}.md`
- **Policy path:** `<charter-root>/policies/<policy>/adapters/<agent>/...`
...
- **Project path:** `<charter-root>/learning/inbox/<date>-<slug>.md`, `<charter-root>/learning/triage.md`, `<charter-root>/archive/<date>-<slug>.md`
...
Source of truth is the `**Active mode:**` line in `<charter-root>/guides/process/modes.md`. Switched by invoking the `mode` action with the desired mode name. Per-agent invocation conventions live in `<charter-root>/adapters/<agent>/lifecycle.md`. Agents without a notion of autonomy levels can ignore modes.

## Agent failure modes

Recurring agent-level failure modes are documented as process guides under `<charter-root>/guides/process/`. They are loaded ambient like any other guide and serve as guardrails the agent reads at session start:
...
These are not their own port — they are guides under the `process` topic — but they form a coherent category worth knowing exists. The category list is curated in `<charter-root>/guides/process/README.md`.

## Learning pipeline

Two-flywheel content evolution:

- **Learning** — `learn` action collects rule candidates from agent sessions and writes them into `<charter-root>/learning/inbox/<date>-<slug>.md`. Each candidate carries evidence. Triage advances candidates through `new → reviewed → accepted | rejected`. Accepted candidates promote to actual guides/corpus.
- **Pruning** — `audit` action identifies stale guides/corpus and moves them into `<charter-root>/archive/<date>-<slug>.md` with a one-line reason. Stale corpus is rare; stale guides are routine as a project's understanding evolves.
...
### Rule 1: Inter-charter references are charter-root-relative

When a markdown file inside the charter links to another charter file,
the link is **relative to the charter root**, not to the source file.
Example:

```markdown
<!-- inside <charter-root>/guides/process/spec.md -->

For reasoning, see [`corpus/process/spec.md`](corpus/process/spec.md).
```

The link resolves against the charter root: `<charter-root>/corpus/process/spec.md`.

**Forbidden:** any `../` or `./` segments in inter-charter links.
...
**Why:** charter files are loaded by an agent that knows the charter
root. Relative-to-source paths force the agent to reconstruct file
locations from imperfect path math; charter-root-relative paths are
...
charter** (the project's source tree, `keystone.json`, the lockfile,
build scripts), the path is **relative to the repo root** — the
directory holding `keystone.json`.

```markdown
<!-- inside <charter-root>/actions/release.md -->

See [`keystone.json`](keystone.json) for the policy pin list.
See [`src/main.go`](src/main.go) for the entrypoint.
```

The charter root is itself reached via `<charter-root>/...` from this
form (e.g. `.charter/guides/process/spec.md` from a code reference).

**Why:** code referenced from a charter file is being read by the agent
to act on the project's source — the agent already operates from the
repo root for those operations.

### Enforcement

- **Generators** emit charter-root-relative inter-charter links and
  repo-root-relative code links by default.
- **`keystone doctor`** flags any inter-charter link containing `../`
