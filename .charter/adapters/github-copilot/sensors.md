...
In VS Code's chat-only (non-agent) mode, the agent surfaces commands as text and the user runs them in a separate terminal. The slow path; the charter still works but every sensor round-trips through the human.
...
The charter's `release.md` describes when to gate on this; the binding above is how Copilot implements it.
