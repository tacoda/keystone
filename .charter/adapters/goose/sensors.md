...
instructions: |
  You are running the verify action. Read .charter/guides/process/verification.md
  for the contract. Sensor commands live in .charter/corpus/state/CODEBASE_STATE.md.
...
- Goose **must have the developer extension enabled** for the charter to be useful; without it, sensors degrade.
- Goose recipes are the closest analog to Claude Code's slash commands; the charter uses them as such.
