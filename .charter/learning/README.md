...
- `inbox/` — raw candidates. The agent writes here when it makes a judgment call not covered by the existing charter.
...
3. Land under the confirmed heading (`## RULES`, `## GOLDEN RULE`, or `## IRON LAW(S)`) inside the target file.
...
## What happened
The situation. What the agent encountered that was not covered by the charter.
