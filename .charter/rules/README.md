...
**framework guides** under `.charter/guides/`. Two different
abstractions, both first-class:

| Layer       | Lives at                                          | When to use it                                                              |
| ----------- | ------------------------------------------------- | --------------------------------------------------------------------------- |
| Framework   | `.charter/guides/<topic>/<name>.md`      | The default. Structured directive with H2 tiers (iron / golden / regular), paired corpus reasoning, severity-tagged. |
| Agent       | `.charter/rules/<id>.md`                 | Plain markdown directive the agent reads as-is. Extends what the host already understands; no tier structure, no paired corpus required. |
