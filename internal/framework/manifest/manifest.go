// Package manifest models a plugin manifest. A manifest is the small
// declarative file at the root of every plugin repo that names it, sets its
...
// else in the cascade (project or any other plugin) can override them.
// `required` items are NOT shipped by this plugin; the plugin declares they
...
// Namespace returns the on-disk directory name used inside harness/plugins/
// for this plugin's content. Always equal to the manifest name.
