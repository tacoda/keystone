# opencode — Sensor binding

How sensors fire inside opencode.

## Execution model

opencode runs shell commands directly. **All sensors run autonomously** —
the agent reads tool commands from `.charter/corpus/state/CODEBASE_STATE.md`,
invokes them via shell, and consumes the output. No human paste-and-report
loop. How aggressively shell runs without approval depends on the
...
opencode supports subagents (projected to `.opencode/agents/`). The
**review** action spawns `review-functional`, `review-security`,
`review-risk`, and `review-deployment` as subagents and combines their
findings — no sequential degradation.

## Stale evidence guard

Same IRON LAW as Claude Code: claims need fresh evidence. The agent must
show the shell output for any sensor it claims passed; without fresh output
in the current session, the claim is invalid.

## Sensor failure handling

1. The **verify** action surfaces structured output from the failing sensor.
2. The agent returns to the **implementation** phase. Do not "fix forward"
   inside verification.
