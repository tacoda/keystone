...
id: charter-debt
description: 'Surfaces debt in the charter itself — content that exists but isn''t earning its keep.'
---
# Sensor: charter-debt

Surfaces **debt in the charter itself** — content that exists but isn't earning its keep. Distinct from [`code-debt`](code-debt.md), which covers debt in the codebase.

- **Trigger** — **audit** (full sweep). Cheaper subsets can run on demand.
- **Inputs** — `.charter/` tree, `.charter/corpus/state/CODEBASE_STATE.md`, `.charter/lockfile.json`, `git log -- .charter/`, and the outputs of the sensors classified as available in `CODEBASE_STATE.md`.
...
- **load-bearing** — the charter gives wrong guidance because of this (rules that contradict current code; sensors that claim to run but don't).
