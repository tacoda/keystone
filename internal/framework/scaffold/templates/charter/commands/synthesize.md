...
After every promotion / removal, run `keystone index` so `.charter/INDEX.json` reflects the new layout. If skills, subagents, or commands changed (rare for synthesize, possible for cross-cutting promotions), also run `keystone project` so host projections track.
