...
How the harness's ambient content (guides and process docs) loads into Goose, and how the agent reaches corpus files on demand.
...
~/.config/goose/hints.md          # global hints (user-owned; harness does not write)
...
Goose's tool use comes from extensions. The harness assumes:
...
Without the developer extension, sensors degrade to "agent surfaces commands; user runs them." That mode works but is not the harness's recommended shape.
...
- **Auto-loaded via recipes:** anything listed in a recipe's `instructions:` field becomes part of the session prompt. The harness's recipes inline pointers to specific phase docs.
...
The harness ships one recipe per lifecycle action. Users can also chain recipes: a `keystone-cycle` recipe that runs `spec → orient → implementation → verify → review` end to end.
...
…and follow the prompts for the MCP server (Atlassian, Linear, GitHub, etc.). The harness assumes presence when documenting tracker flows; without the extension, the **tracker-card-fetcher** sensor falls back to shell (`gh issue view`) or user paste.
...
After a flywheel write that touched any `harness/guides/` file, start a new session so the next turn re-reads the updated guides via `.goosehints` and any recipe instructions.
...
- **No autonomy levels.** Interactive vs. recipe is the only axis; the harness's `paired`/`solo`/`autopilot` collapse approximately.
- **No native lazy-by-region.** Idiom loading is explicit, done inside the orient action.
- **Provider lock to the global config.** Goose's model provider is global, not per-project. A team standardizing on a model needs documentation outside the harness.
