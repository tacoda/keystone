...
How the harness's ambient content (guides and process docs) loads into Cline, and how the agent reaches corpus files on demand.
...
.roorules              # same, for Roo Code
                       # (the harness ships cline-instructions.md as paste source)
```
...
Cline's **workflows** feature stores reusable prompts triggered from the side panel. The harness recommends one workflow per lifecycle action — see [`lifecycle.md`](lifecycle.md) for the full list. Users add these via the workflows menu in the side panel; the installer cannot create them programmatically.

## Ambient loading

Cline's ambient surface:

- **Auto-loaded every session:** the custom-instructions field (extension settings) and `.clinerules` (workspace, if newer Cline). The harness uses both — the settings field for safety, `.clinerules` for users on supported versions.
...
Cline has first-class MCP support. The harness recommends configuring at minimum:
...
MCP servers are configured in the extension's MCP settings panel (the gear icon). The harness does not install MCP configuration — the user enables and configures servers themselves; the harness assumes presence when documenting tracker / GitHub flows.
...
After a flywheel write that touched any `harness/guides/` file, start a new task so the next turn re-reads the updated guides via the custom-instructions field.
...
Cline's per-command approval can stall sensor execution. The harness recommends auto-approving at minimum:
...
- **Roo Code drift.** Roo Code forks Cline and diverges over time. The harness adapter applies to both, but specific feature names (workflows, custom-instructions field key) may differ — check the Roo Code docs if a binding does not work.
