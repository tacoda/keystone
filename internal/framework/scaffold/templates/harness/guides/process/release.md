...
Commit messages, PR descriptions, and tracker tickets do not mention Claude, an AI agent, or any tool used to author the change. The author is the human running the harness.
...
Invoke the **learn** action. If during the work the agent encountered a situation not covered by the corpus and made a judgment call, the reasoning gets captured to `harness/learning/inbox/` for the next **synthesize** cycle.
...
- **Neither exists** → the harness still works for code quality, but release surfaces a warning: shipping without CI is a known gap. Recommend wiring at least lint + test into CI before relying on the **verify** action's evidence.
...
| Learning capture | `harness/learning/inbox/<timestamp>-<slug>.md` |
