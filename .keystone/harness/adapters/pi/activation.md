...
The installer does **not** create `.pi/`. Lifecycle actions are agent-agnostic playbooks at `harness/actions/<action>.md`; the agent reads them on demand when the user asks to run an action by name.

## Ambient loading

Pi's `AGENTS.md` precedence chain means the corpus pointer is always available regardless of where in the repo the user runs pi. The full corpus (`harness/`) is read on demand via pi's file-read capability — not autoloaded.
