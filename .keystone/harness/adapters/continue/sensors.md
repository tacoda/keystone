...
**Sensors run autonomously** in agent mode (with per-command approval) — the user is not pasting output back from a separate terminal. The approval prompt is the gate; sensor commands declared in the harness are deterministic and safe to allow.
...
Pair with `scripts/verify.sh` in the project; the harness recommends this when the verify cycle exceeds ~30 seconds wall-clock per sensor.
