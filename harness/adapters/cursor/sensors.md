...
- **Chat / ask mode** — the agent surfaces commands as text; the user runs them in a terminal and pastes the output back. The slow path; the harness still works but every sensor round-trips through the human.

The per-command approval in agent mode is Cursor's only autonomy lever. Even in "auto-accept" toggles, the harness assumes the user is reviewing each shell command before it runs.
