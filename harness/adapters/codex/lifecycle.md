...
| Action | Invocation |
|---|---|
| **spec** | "Start the spec phase for `<task or tracker card>`." Codex reads `harness/process/spec.md` and follows it. |
...
| **mode** | "Set pacing mode to `<paired\|solo\|autopilot>`." Edit `harness/process/modes.md` directly. |
...
`harness/process/modes.md` documents this mapping in the `mode` action body.
