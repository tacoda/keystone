...
The **verify** action's IRON LAW — "no completion claims without fresh verification evidence" — is enforced by re-running each sensor in the current turn. The agent must show the Bash tool output for sensors it claims passed; the harness review agent (`review-functional`) flags claims without matching tool calls in the turn's transcript.
